  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-sm font-medium" style={{ color: "#f5f3ee" }}>{label}</span>
        <span className="text-sm font-bold" style={{ color: "#c49a5a" }}>{Math.round(value)}</span>
      </div>
      <div className="h-2 rounded-full overflow-hidden" style={{ background: "#1d1d24" }}>
        <div className="h-full rounded-full" style={{ width: `${value}%`, background: "#a07840" }} />
      </div>
      <p className="text-xs mt-1" style={{ color: "#5c5c64" }}>{hint}</p>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0c0c0e", color: "#f5f3ee" }}>
      ...
      <main className="px-6 py-10 max-w-[1000px] mx-auto">
        <Link href="/resources/top-products" className="text-sm transition-colors hover:text-[#c49a5a]" style={{ color: "#8a8a94" }}>
        ...
        <div className="flex items-start gap-5 mt-6 mb-8 flex-wrap">
          <div className="flex items-center justify-center w-20 h-20 rounded-2xl text-4xl shrink-0"
            style={{ background: "#15151a", border: "1px solid #2a2a33" }}>{p.emoji}</div>
          <div className="flex-1 min-w-[200px]">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="rounded-full px-2.5 py-0.5 text-xs font-semibold"
                style={{ background: "#1d1d24", color: "#8a8a94" }}>{p.niche}</span>
              {p.is_featured && (
          ...
          <div className="text-center rounded-2xl px-6 py-4" style={{ background: "#15151a", border: `1px solid ${band.color}55` }}>
            ...
            <p className="text-[10px] mt-0.5" style={{ color: "#5c5c64" }}>IQ Score</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-2xl p-6" style={{ background: "#15151a", border: "1px solid #2a2a33" }}>
              <h2 className="font-bold text-sm mb-4" style={{ color: "#f5f3ee" }}>Key metrics</h2>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {stats.map((s) => (
                  <div key={s.label} className="rounded-xl p-3" style={{ background: "#1d1d24" }}>
                    <p className="font-bold text-sm" style={{ color: "#f5f3ee" }}>{s.value}</p>
                    <p className="text-[11px] mt-0.5" style={{ color: "#5c5c64" }}>{s.label}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-2xl p-6 space-y-4" style={{ background: "#15151a", border: "1px solid #2a2a33" }}>
              <h2 className="font-bold text-sm" style={{ color: "#f5f3ee" }}>IQ Score breakdown</h2>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl p-6" style={{ background: "#15151a", border: "1px solid #2a2a33" }}>
              <h2 className="font-bold text-sm mb-4" style={{ color: "#f5f3ee" }}>Find a supplier</h2>
              <div className="space-y-2">
                {p.suppliers.map((s) => (
                  <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-between rounded-xl px-3 py-2.5 transition-colors"
                    style={{ background: "#1d1d24", border: "1px solid #2a2a33" }}>
                    <span className="text-sm font-medium" style={{ color: "#f5f3ee" }}>{s.name}</span>
                    <span className="text-sm font-bold" style={{ color: "#5eb89a" }}>~{money(s.price)} →</span>
                  </a>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-2xl p-6" style={{ background: "#15151a", border: "1px solid #2a2a33" }}>
              <h2 className="font-bold text-sm mb-4" style={{ color: "#f5f3ee" }}>Stores selling this</h2>
              <div className="space-y-2">
                {p.top_stores.map((s) => (
                  <div key={s.domain} className="rounded-xl px-3 py-2.5" style={{ background: "#1d1d24" }}>
                    <p className="text-sm font-medium" style={{ color: "#f5f3ee" }}>{s.name}</p>
