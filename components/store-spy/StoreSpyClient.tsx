    ...
    <div>
      ...
      <div className="rounded-2xl p-5 mb-6" style={card}>
        <div className="flex gap-3 flex-col sm:flex-row">
          ...
          <button
            ...
            style={{ background: loading || !url.trim() ? "#2a2a33" : "#a07840", color: loading || !url.trim() ? "#5c5c64" : "#f5f3ee" }}
          >
        ...
        <div className="flex items-center gap-2 mt-3 flex-wrap">
          ...
          {EXAMPLES.map((ex) => (
            <button key={ex} onClick={() => analyze(ex)}
              ...
              style={{ background: "#1d1d24", color: "#8a8a94", border: "1px solid #2a2a33" }}>
              {ex}
            </button>
          ))}
        </div>
      ...
      {loading && (
        <div className="rounded-2xl p-12 text-center" style={card}>
          <p className="text-sm" style={{ color: "#8a8a94" }}>⏳ Fetching products & detecting apps…</p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl p-6" style={card}>
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                <h2 className="font-bold" style={{ fontSize: 22, color: "#f5f3ee" }}>{result.store_name}</h2>
                <p className="text-sm" style={{ color: "#8a8a94" }}>
                  {result.store_url.replace("https://", "")} · <span style={{ color: "#c49a5a" }}>{result.niche}</span>
                ...
                <div className="flex gap-2">
                  {Object.entries(result.social_links).map(([k, v]) => (
                    <a key={k} href={v} target="_blank" rel="noopener noreferrer"
                      className="text-xs rounded-full px-2.5 py-1" style={{ background: "#1d1d24", color: "#8a8a94" }}>{k}</a>
                  ))}
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Est. monthly revenue", value: money(result.monthly_revenue_est), accent: "#5eb89a" },
              { label: "Est. monthly traffic", value: result.monthly_traffic_est.toLocaleString(), accent: "#c49a5a" },
              { label: "Est. ad spend / mo", value: money(result.monthly_ad_spend_est), accent: "#d4b572" },
              { label: "Products", value: result.product_count.toLocaleString(), accent: "#f5f3ee" },
            ].map((s) => (
              <div key={s.label} className="rounded-2xl p-5" style={card}>
                ...
                <p className="text-xs mt-1" style={{ color: "#8a8a94" }}>{s.label}</p>
              </div>
            ))}
          </div>
          ...
          {result.top_products.length > 0 && (
            <div className="rounded-2xl p-6" style={card}>
              <h3 className="font-bold text-sm mb-4" style={{ color: "#f5f3ee" }}>Top products</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {result.top_products.map((p) => (
                  <div key={p.handle} className="rounded-xl overflow-hidden" style={{ background: "#1d1d24" }}>
                    {p.image ? (
                    ...
                    <div className="p-3">
                      <p className="text-xs font-medium truncate" style={{ color: "#f5f3ee" }}>{p.title}</p>
                      <p className="text-xs font-bold mt-1" style={{ color: "#5eb89a" }}>{money(p.price)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Installed apps */}
          <div className="rounded-2xl p-6" style={card}>
            <h3 className="font-bold text-sm mb-4" style={{ color: "#f5f3ee" }}>
              Installed apps {result.installed_apps.length > 0 && `(${result.installed_apps.length})`}
            </h3>
            {result.installed_apps.length === 0 ? (
              <p className="text-sm" style={{ color: "#8a8a94" }}>No known apps detected on the homepage.</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {result.installed_apps.map((a) => (
                  <span key={a.name} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs"
                    ...
                    <span style={{ color: "#5c5c64" }}>· {a.category}</span>
                  </span>
                ))}
              </div>
            )}
