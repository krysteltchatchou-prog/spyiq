    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-2 space-y-5">
        ...
        <div className="rounded-2xl p-5" style={card}>
          <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "#8a8a94" }}>Generator</p>
          <div className="grid grid-cols-2 gap-2">
            {GENERATORS.map((g) => (
              <button key={g.id} onClick={() => setType(g.id)}
                ...
                style={{
                  background: type === g.id ? "rgba(160,120,64,0.12)" : "#1d1d24",
                  border: `1px solid ${type === g.id ? "#a07840" : "#2a2a33"}`,
                  color: type === g.id ? "#c49a5a" : "#8a8a94",
                }}>
        ...
        <div className="rounded-2xl p-5 space-y-4" style={card}>
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "#8a8a94" }}>Product / store name</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "#8a8a94" }}>Tone</label>
            <div className="flex flex-wrap gap-2">
              {TONES.map((t) => (
                <button key={t} onClick={() => setTone(t)}
                  className="rounded-full px-3 py-1.5 text-xs font-semibold transition-all"
                  style={{
                    background: tone === t ? "#a07840" : "#1d1d24",
                    border: `1px solid ${tone === t ? "#a07840" : "#2a2a33"}`,
                    color: tone === t ? "#f5f3ee" : "#8a8a94",
                  }}>{t}</button>
              ))}
            </div>
          </div>
          ...
          <button onClick={generate} disabled={loading}
            ...
            style={{ background: loading ? "#2a2a33" : "#a07840", color: loading ? "#5c5c64" : "#f5f3ee" }}>
            {loading ? "Generating…" : "✨ Generate"}
          </button>

          {credits && (
            <p className="text-xs text-center" style={{ color: "#5c5c64" }}>
              AI credits: <span style={{ color: "#c49a5a" }}>{Math.max(0, credits.limit - credits.used)}</span> of {credits.limit} remaining
            </p>
          )}
        </div>
      </div>
      ...
      <div className="lg:col-span-3">
        <div className="rounded-2xl p-6 min-h-[400px] flex flex-col" style={card}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-sm" style={{ color: "#f5f3ee" }}>
              {GENERATORS.find((g) => g.id === type)?.label} · <span style={{ color: "#8a8a94" }}>{tone}</span>
            </h3>
            {output && (
              <div className="flex gap-2">
                <button onClick={copyOutput}
                  className="rounded-lg px-3 py-1.5 text-xs font-semibold transition-all"
                  style={{ background: copied ? "rgba(94,184,154,0.15)" : "#1d1d24", border: "1px solid #2a2a33", color: copied ? "#5eb89a" : "#c49a5a" }}>
                  {copied ? "✓ Copied" : "📋 Copy"}
                </button>
                <button onClick={generate} disabled={loading}
                  className="rounded-lg px-3 py-1.5 text-xs font-semibold transition-all"
          ...
          {loading ? (
            <div className="flex-1 flex items-center justify-center">
              <p className="text-sm" style={{ color: "#8a8a94" }}>✨ Writing with market context…</p>
            </div>
          ) : output ? (
            <pre className="text-sm leading-relaxed whitespace-pre-wrap flex-1 font-sans" style={{ color: "#f5f3ee" }}>{output}</pre>
          ) : (
            <div className="flex-1 flex items-center justify-center text-center">
              <p className="text-sm max-w-[280px]" style={{ color: "#5c5c64" }}>
